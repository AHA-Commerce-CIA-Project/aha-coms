...
> **Live in production as of 2026-05-20** (revision `coms-portal-api-00028-nxz`, applied via `tofu apply` of `infra/cloud-run.tf` adding the `FORCE_PASSWORD_SETUP_ENABLED=true` env block on the `coms-portal-api` Cloud Run service — PR #64). Convergence is underway; every existing identity with `password_set_at IS NULL` is now routed to `/onboarding/set-password` on their next sign-in. The "Rollout discipline" paragraph below describes the dry-run + flip procedure that was followed; the section's later acceptance criteria are now satisfied in production rather than planned.
