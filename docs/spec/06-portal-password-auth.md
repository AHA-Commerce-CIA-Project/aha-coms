...
> Status: **shipped 2026-05-20** — PR F landed across the corridor of commits `eb13d13` → `cd5d593` → `7d65a72` → `ba83444` → `f1e143e`, with the in-flight rollout pieces (PR #64 / #71 / #72 / #73) cited at the points of incidence below. Every `[x]` checkbox in *Success criteria* + *Phases 1–5* records a fact about main as of the spec-reconciliation sweep on 2026-05-21; the inline SHAs cite the load-bearing commit for each. Original authoring trigger: `tasks/todo.md` FU-14 ("Portal admin affordance — 'create identity with arbitrary email + password'") plus the in-session expansion to a user-side password sign-in option.
...
- [x] (PR F: f1e143e) `POST /api/auth/password/sign-in` exists on portal-api. Accepts `{ email, password }`. Verifies via GIP REST (`accounts:signInWithPassword`). On success mints a portal session cookie (same `authMethod` field shape as `personal_otp` / `workspace_oidc`, value `'password'`). Returns 401 with `{ error: 'INVALID_CREDENTIALS' }` on bad password; 403 with `{ error: 'INACTIVE_USER' }` on disabled identity; 429 with `Retry-After` on rate-limit hit.
- [x] (PR F: f1e143e) `POST /api/auth/password/set` exists on portal-api. Accepts `{ currentPassword?, newPassword }` (current-password is optional for the first-set case, required for subsequent change-password calls). Validates `newPassword` against the policy below. Calls GIP REST (`accounts:update` with `password`) to apply. On success, records the audit event `password_set` (no password value in the payload).
- [x] (PR F: f1e143e) `POST /v1/identities` exists on portal-api as the sibling-to-`/v1/employees` admin endpoint. Body: `{ name, email, password, notes? }` (kind is hard-coded to `'personal'` per §2; no `kind` field on the wire). Only `requireRole('admin')` callers pass. Creates a lean `identity_users` row (no phone/dept/position/branch/leader/teamId required) with `password_only_auth = TRUE`, one `identity_user_emails` row with `kind = 'personal'` + `addedBy = 'admin'`, calls `createGipUser(email, password)` directly (no random-temp + reset-link dance — admin-supplied password lands verbatim). Returns the new identity's id + initial provisioning state.
- [x] (PR F: f1e143e) `identity_users.notes` column added (TEXT, nullable). Migration 0033 (or next sequential — verify at implementation time). Captured for the audit-trail use case named in FU-14 recommended-shape §3.
- [x] (PR F: f1e143e) `identity_users.password_set_at` column added (timestamp, nullable). Set to `now()` whenever GIP password is updated via `/api/auth/password/set` or `POST /v1/identities`. Read by the first-login flow to decide whether to force the password-setup prompt.
- [x] (PR F: f1e143e) `identity_users.password_only_auth` column added (BOOLEAN NOT NULL DEFAULT FALSE). Set to `TRUE` by `POST /v1/identities`. Read by `POST /api/auth/otp/request` (returns `{ error: 'PASSWORD_ONLY' }` when true) and by the forgot-password handler (refuses to issue reset link when true).
- [x] (PR F: f1e143e) `identity_users.password_lockout_until` column added (timestamp, nullable). Written by `POST /api/auth/password/sign-in` when the 5-failures-in-10-minutes threshold trips. Cleared on first successful sign-in.
- [x] (PR F: f1e143e) `apps/portal-web/src/routes/login/+page.svelte` extends to a four-step flow: `choose → email → password|otp`. The `choose` and `email` steps stay as-is; the third step routes to `otp` if the email is OTP-only or `password` if a password has been set. A `forgot-password` link sits on the `password` step (reuses the existing `generatePasswordResetLink` REST helper at `apps/portal-api/src/gip-admin.ts:77` — already wired, no new code).
- [x] (PR F: f1e143e) First-login forced-set: when an identity signs in for the first time AND has no `password_set_at`, the session is minted but flagged `requires_password_setup`. portal-web's `(authed)` layout reads the flag, redirects to a one-time `/onboarding/set-password` route, blocks all other routes until the user POSTs to `/api/auth/password/set`. On success, the flag clears.
- [x] (PR F: f1e143e) Password policy enforced server-side at both `POST /api/auth/password/set` and `POST /v1/identities`: **minimum 12 characters, no composition rules.** Revised from the original "8 chars + letter + digit" gate on 2026-05-19 to match NIST SP 800-63B and OWASP ASVS L2 guidance — composition rules are research-debunked (users defeat them trivially with `Password1`, `Pa$$w0rd1` etc. without raising actual strength). Length plus rate-limiting plus zxcvbn-based UI feedback do the real work. The server rejects shorter passwords with `{ error: 'WEAK_PASSWORD', message }`. The portal-web meter (`password-strength-meter.svelte`) uses **zxcvbn-ts** for pattern-aware scoring — catches dictionary words, sequences, keyboard runs, leet substitution, personal-info matches (email/name passed as `userInputs`), and displays an estimated offline crack time. zxcvbn score 0-1 surfaces as "weak/very weak", 2 as "fair", 3-4 as "strong/very strong". The score is a UI hint only; the server enforces only length.
- [x] (PR F: f1e143e) New `apps/portal-web/src/routes/(authed)/admin/identities/` route exists. List view shows all `identity_users` with `source = 'manual'`. Create form: name + email + kind + password (cleartext field with show/hide toggle + strength meter) + notes. Calls `POST /v1/identities`. Audit-log links from the row to the create event.
- [x] (PR F: f1e143e) Audit log records `create_identity_with_password` (admin path) and `password_set` (user path) actions; details payload includes actor, target email, kind, but never the password.
- [x] (PR F: f1e143e) Tests cover: GIP password-sign-in happy path; bad password 401; inactive-user 403; rate-limit 429; password policy rejection (too short / no digit / no letter); admin-create-identity happy path; first-login forced-set redirect; subsequent change-password requires `currentPassword`.
- [x] (PR F: f1e143e) No regression in existing Google sign-in or OTP flows (verified by running existing test suites + a manual smoke).
- [x] (PR F: f1e143e) ADR 0006 (GIP-only auth) reconciled — see below.
- [x] (PR F: f1e143e) `tasks/todo.md` FU-14 marker flips `[ ]` → `[x]` with a Done block citing the new spec + the commits that implement it.
...
- [x] (PR F: f1e143e) **T01: Migration 0033 — add four columns to `identity_users`.**
...
- [x] (PR F: f1e143e) **T02: Author the password-policy validator.**
...
- [x] (PR F: f1e143e) **T03: Author `apps/portal-api/src/services/identities.ts`.**
...
- [x] (PR F: f1e143e) **T04: Author `apps/portal-api/src/routes/identities.ts`.**
...
- [x] (PR F: f1e143e) **T05: Author the portal-web `/admin/identities` route.**
...
- [x] (PR F: f1e143e) **T06: Author `POST /api/auth/password/sign-in`.**
...
- [x] (PR F: f1e143e) **T07: Extend `POST /api/auth/otp/request` with two new short-circuit outcomes.**
...
- [x] (PR F: f1e143e) **T08: Extend `apps/portal-web/src/routes/login/+page.svelte` to a four-step flow.**
...
> **Live in production as of 2026-05-20.** Rolled out across three PRs:
>
> - PR #64 — added the `FORCE_PASSWORD_SETUP_ENABLED=true` env block to `coms-portal-api` in `infra/cloud-run.tf`. Initial flip.
> - PR #71 — relocated the flag into `local.portal_shared_env` so `coms-portal-web` also receives it. Required because `hooks.server.ts` validates sessions in-process per the FU-12 loopback seal (commit 26841b8); the IIFE that computes `passwordSetupRequired` reads `process.env` inside whatever container imports it, and a flag set on the API alone could never light the SSR redirect. Without this, users who logged out and back in saw no setup window.
> - PR #73 — bound `roles/firebaseauth.admin` to `coms-portal-run-sa` (the runtime SA). `apps/portal-api/src/gip-admin.ts` authenticates Identity Toolkit Admin calls with the runtime SA's ADC token; without the role, `accounts:update` returns `INSUFFICIENT_PERMISSION` (HTTP 400), the route throws an unguarded `Error`, and the global onError handler sanitises it to "Internal error". The role had previously been bound only to the orphaned `coms-portal-workspace-sync` SA (see `infra/iam-firebase-auth.tf`) — the binding was never relocated when that feature was removed.
>
> Convergence is underway; every existing identity with `password_set_at IS NULL` is routed to `/onboarding/set-password` on their next sign-in. The "Rollout discipline" paragraph below describes the dry-run + flip procedure that was followed; the section's later acceptance criteria are now satisfied in production rather than planned.
...
- [x] (PR F: f1e143e) **T09: Author `POST /api/auth/password/set`.**
...
- [x] **T10: Author `apps/portal-web/src/routes/(authed)/onboarding/set-password/+page.svelte`.**
  ...
  - **Tip card (PR #72):** an info card between the intro paragraph and the form names the 12-character floor, recommends a 4+ unrelated-word passphrase, and gives two concrete examples (`coffee mountain candle truck`, `silver-river-42-window`). Same copy mirrored onto the `/portal/profile` rotation flow below.
  - Acceptance: manual smoke covers the forced-redirect from `(authed)` layout + the post-set redirect.

- [x] **T10a: Author the rotation flow on `apps/portal-web/src/routes/(authed)/profile/+page.svelte` (PR #72).**
  - Card visible only when `user.passwordSetupRequired === false` (post-convergence users). Three fields — current / new / confirm — call `setPassword(newPassword, currentPassword)` from `$lib/auth`, mapping every server discriminator (`WEAK_PASSWORD` / `CURRENT_PASSWORD_REQUIRED` / `CURRENT_PASSWORD_INVALID` / network error) to inline messages. The same `PasswordStrengthMeter` from onboarding rides under the New-password input, with `userInputs` wired to the user's email + name so identity-derived passwords still get penalised.
  - The client-side `SessionUser` interface in `apps/portal-web/src/lib/auth.ts` carries `passwordSetupRequired?: boolean` so the gate can be read from `$page.data.user`; the underlying runtime value was already arriving through `/api/auth/me`'s payload (PR F §11) before the type was widened.

- [x] (PR F: f1e143e) **T11: Wire the `requires_password_setup` flag into the session payload + the `(authed)` layout guard.**
  ...
  - Layout guard: `apps/portal-web/src/hooks.server.ts:70-75` reads the flag from the resolved `AuthUser`; if true, redirects every authed route except `/onboarding/set-password` to that route. (The original spec drafted this on `+layout.server.ts` but the hook is the actual implementation since the FU-12 loopback seal moved session validation in-process.)
...
- [x] (PR F: f1e143e) **T12: Test suite — integration sweep.**
...
- [x] (PR F: f1e143e) **T13: ADR 0006 addendum.**
...
- [x] (PR F: f1e143e) **T14: CONTEXT.md update.**
...
- [x] (PR F: f1e143e) **T15: FU-14 closure.**
...
- **Hidden IAM prerequisite (post-mortem note added 2026-05-20).** Every admin-scoped Identity Toolkit call in `apps/portal-api/src/gip-admin.ts` — `updateGipUserPassword`, `generatePasswordResetLink`, `revokeRefreshTokens`, `setGipUserDisabled` — authenticates with the runtime SA's ADC bearer. The runtime SA must therefore hold `roles/firebaseauth.admin` on the project. Without it, GIP returns `INSUFFICIENT_PERMISSION` and the global onError handler at `apps/portal-api/src/middleware/api-error-handler.ts:47` sanitises every failure to a flat "Internal error". The role binding was originally provisioned only on the (later-removed) `coms-portal-workspace-sync` SA and the relocation to `coms-portal-run-sa` only landed in PR #73, after the first user pressed Save on the forced-set screen and the error surfaced in logs. Any future runtime-SA reprovisioning, project mirror, or replica must restore this binding — see `infra/iam-portal-runtime.tf`.
