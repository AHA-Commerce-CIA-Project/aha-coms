...
- [ ] Password policy enforced server-side at both `POST /api/auth/password/set` and `POST /v1/identities`: **minimum 8 characters, must contain at least one letter AND at least one digit**. **Strong (UI hint, not enforced):** additional capital letter + special character (`@` `!` `#` `$` `%` `^` `&` `*`). The minimum is rejected with `{ error: 'WEAK_PASSWORD', message }`; the strong hint surfaces in the portal-web password-setup component as a real-time strength meter.
...
- [ ] **T02: Author the password-policy validator.**
  - File: `apps/portal-api/src/services/password-policy.ts` (new). Pure function. Two exports: `validateMinimum(pwd: string): { ok: true } | { ok: false; reason: string }` (≥8 chars + ≥1 letter + ≥1 digit) + `scoreStrength(pwd: string): 'weak' | 'fair' | 'strong'` (returns the strength tier for the UI hint; uses presence of capital letter + special char to lift `fair → strong`).
  - Acceptance: unit tests cover the boundary cases (7 chars rejected, no-digit rejected, no-letter rejected, exactly-8-with-mix accepted; `scoreStrength` returns `weak`/`fair`/`strong` on the three named tiers).
...
- [ ] **T05: Author the portal-web `/admin/identities` route.**
  ...
  - Form fields: name + email + password (with show/hide toggle + real-time strength meter) + optional notes. No kind radio (hard-coded `'personal'` server-side). Uses Eden RPC to POST to `/v1/identities`. Strength meter calls a small client-side mirror of `scoreStrength` (lives in `packages/shared` as `passwordStrength.ts`); SERVER-side `validateMinimum` is the authoritative gate, client mirror is UX-only.
...
**Rollout discipline:** Phase 4 deploys behind a feature flag `FORCE_PASSWORD_SETUP_ENABLED` so the schema migration (T01) + service code (T03–T09) can ship without immediately triggering the prompt suite-wide. Flip the flag once T10 + T11 are merged and the engineer has run a single dry-run sign-in on a staging identity. Document the flip in `tasks/todo.md`.
