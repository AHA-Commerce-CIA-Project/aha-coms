...
All four are now closed (F10, F11, F12, F13 — all same day as ship). No Spec 02 thread remains warm. The CI drift guard is fully live: F13b codified the deployer SA's `roles/viewer` + `roles/storage.objectViewer` grants in `infra/modules/github-wif/main.tf` (applied same day) and the GitHub repo variables (`INFRA_PLAN_GUARD_ENABLED`, `ALERT_EMAIL`, `SHEET_ID_POINTS`, `SHEET_ID_EMPLOYEES`) are set.
...
7. **Commit-message hygiene for issue closure.** F10/F11 shipped together as portal commit `3f1d34c` with subject "Close Spec 02 follow-ups #3 and #4 — generalize route canary, isolate manifests test". GitHub auto-close did **not** fire — the closing keyword must directly precede `#N` (e.g. `Closes #3` on its own line), and chaining `#3 and #4` after intervening text doesn't match the grammar either. Both issues stayed OPEN through push to `main` and were only reconciled later 2026-05-07 with `gh issue close --comment` referencing the F10/F11 commit. Future ledger commits should use one `Closes #N` trailer per issue, on its own line near the bottom of the body, and verify the close fired (`gh issue view N --json state`) before treating the loop as closed.
