...
> Status: **Cutover EXECUTED 2026-05-05 — Heroes Deploy A is LIVE IN PROD.** Heroes-side ALL SHIPPED 2026-05-04 (PR A1 commit `57fd523` + PR A2 across 8 slices in 11 commits + 2 deploy fixes `a7f9ed9` + `f62f2be`); Heroes CI ✅ + Deploy ✅ on `f62f2be` (run `25314176044`); final gate server typecheck=0, web typecheck=0/5915 files, tests=67 pass / 0 fail, ci:check-no-illegal-inserts=0 across 174 files. **Cutover window EXECUTED 2026-05-05 against prod directly** per the prod-as-rehearsal decision. Cutover sequence collapsed onto cutover-day smoke-tests: T-0 TRUNCATE skipped (Heroes prod already in cutover-target state — `heroes_profiles=72, taxonomy_cache=15, email_cache=72`); final rebroadcast `dispatched:72, skipped:0, failed:0`; cutover-verify Check 1 PASS (72=72), Check 2 PASS (branches:2/2 + teams:13/13), Check 3 PASS (0 pre-step5 pending_alias rows), Check 4 PASS (5 spot-checked employment blocks match portal); T+30 Deploy C verified **NON-APPLICABLE** (`heroes_app_role` does not exist on cluster — Heroes has zero direct portal-DB grants by virtue of separate Cloud SQL users; §Success Criteria #7 holds via absence-of-grants rather than presence-of-revokes; cutover migration README updated with probe transcript). Closeout commits on portal today: `ba1983a` (PR 07-3.5 rebroadcast endpoint), `66a46d3` (rebroadcast metric tightening), `d7112db` (TODO sync — pre-cutover prep complete), `049252b` (cutover closeout). Portal PR 07-5 SHIPPED 2026-05-06 (commit `27aed23`, CI run `25415453742`) — burn-in gate waived; dropped legacy duplicates `email`/`branch` (kept `appRole` as canonical per-app role broadcast — re-classified at ship-time since the v1.6.0 envelope has no role field), bumped shared pin to v1.6.0, forced manifest `schemaVersion: 2` floor (migration `0033_nifty_raza.sql`). Same-day follow-up `29966d7` emptied Heroes' manifest `config_schema` (`leaderboard_eligible` + `starting_points` had zero consumers in Heroes; migration `0034_drop_heroes_deadconfig.sql`). Pending: Heroes-side Phase 6 cleanup only.
...
| **3.5** | Portal | ✅ SHIPPED 2026-05-06 (Spec 07 PR 07-5, commit `27aed23`). Dropped legacy duplicates `email`/`branch`; **kept `appRole`** (re-classified as the canonical per-app role broadcast since the v1.6.0 envelope has no role field). Burn-in gate waived after day-1 fixes stabilised. |
