...
- **Spec 03 portal-side has shipped (2026-04-28).** Heroes-side adoption per §Appendix A of `spec-03-user-identity-alias-layer.md` is now unblocked; both teams can sequence the cutover whenever capacity opens. Note the test-gate caveat in `spec-03b-test-gate-cleanup.md` — staging stays on the pre-Spec-03 release until the gate clears, but the source on `main` is current and the new endpoints (resolve-batch, GET config, app-token middleware) are merged.
