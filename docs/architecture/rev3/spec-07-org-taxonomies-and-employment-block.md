...
> Status: **Cutover EXECUTED 2026-05-05 — Heroes Deploy A is LIVE IN PROD. PRs 07-1 / 07-2 / 07-3 / 07-4 SHIPPED 2026-05-04 (`@coms-portal/shared` v1.6.0 published); PR 07-3.5 (cutover-window blocker — bulk rebroadcast endpoint) SHIPPED 2026-05-05 in two commits (`ba1983a` initial endpoint + `66a46d3` metric tightening — `dispatched`/`skipped`/`failed` distinction). Cutover sequence: pre-flight done via portal admin API (taxonomies seeded, all-staff team granting Heroes app, 71 branches PATCHed); T-0 TRUNCATE skipped (Heroes prod was already in cutover-target state from same-day smoke tests via live webhook handlers); cutover-verify 4/4 automated checks PASS; T+30 Deploy C verified non-applicable (`heroes_app_role` does not exist on cluster — Heroes has zero portal-DB grants by virtue of separate Cloud SQL users; §Success Criteria #7 holds via absence-of-grants). Closeout commits today: `ba1983a`, `66a46d3`, `d7112db`, `049252b`. PR 07-5 (drop legacy emit fields, bump shared pin to v1.6.0, force manifest schemaVersion:2) is the only remaining portal-side work — gated on ~7d Heroes Deploy A burn-in.**
