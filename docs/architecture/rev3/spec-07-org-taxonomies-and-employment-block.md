...
> Status: **Spec 07 PORTAL-SIDE COMPLETE — all five PRs shipped + deployed. PRs 07-1 / 07-2 / 07-3 / 07-4 SHIPPED 2026-05-04 (`@coms-portal/shared` v1.6.0 published); PR 07-3.5 (cutover-window blocker — bulk rebroadcast endpoint) SHIPPED 2026-05-05 (`ba1983a` + `66a46d3`). Cutover EXECUTED 2026-05-05 — Heroes Deploy A LIVE IN PROD (T-0 TRUNCATE skipped; cutover-verify 4/4 PASS; Deploy C non-applicable — Heroes has zero portal-DB grants by virtue of separate Cloud SQL users). **PR 07-5 SHIPPED 2026-05-06** (commit `27aed23`, CI run `25415453742`) — 7-day burn-in gate waived after day-1 fixes stabilised. Dropped legacy duplicates `email` (→ `contactEmail`) and `branch` (→ `employment.branch.value_snapshot`). **`appRole` retained** — re-classified at ship-time as the canonical per-app role broadcast since the v1.6.0 envelope has no role field; Heroes' `handle-user-updated` mirrors `payload.appRole` into `heroes_profiles.role`. Bumped `@coms-portal/shared` git+url pin v1.5.0 → v1.6.0 in `apps/api/package.json` + `apps/web/package.json`. Replaced local payload type declarations in `taxonomy-events.ts` / `employment-resolution.ts` / `provisioning-events.ts` with shared imports. Forced manifest `schemaVersion: 2` floor (column default + `registerApp` validator + Elysia route schema; migration `0033_nifty_raza.sql` forward-fills v1 rows). Same-day follow-up `29966d7` emptied Heroes' manifest `config_schema` after surfacing that `leaderboard_eligible` + `starting_points` had zero consumers in Heroes (migration `0034_drop_heroes_deadconfig.sql`). 555/555 API tests pass; api + web tsc clean.**
...
4. ✅ `taxonomy.upserted` / `taxonomy.deleted` emit machinery shipped, gated by `ENABLE_TAXONOMY_EVENTS` env flag (PR 07-2). Admin-route callers wired in PR 07-2 (single + bulk paths); race-window invariant covered by regression tests (PR 07-3). **Flag flipped to `true` in production 2026-05-05** (`infra/cloud-run.tf:120-123`); end-to-end webhook delivery verified the same day (portal admin upsert → Heroes prod `taxonomy_cache`).
...
7. ✅ Portal dual-emitted legacy + new envelope during gap window (PR 07-3 for `user.provisioned`; 2026-05-06 burn-in fix for `user.updated`). **PR 07-5 SHIPPED 2026-05-06 (commit `27aed23`)** — dropped legacy duplicates `email` (→ `contactEmail`) and `branch` (→ `employment.branch.value_snapshot`). **`appRole` retained** as the canonical per-app role broadcast (re-classified from "legacy" at ship-time — the v1.6.0 envelope has no role field, and Heroes' `handle-user-updated` mirrors it into `heroes_profiles.role`).
8. ✅ `@coms-portal/shared` v1.6.0 published with new types (PR 07-4 SHIPPED 2026-05-04, commit `19cf057` on `mrdoorba/coms-shared`, tag `v1.6.0`). v1.5.0 backcompat locked by 12 verbatim tests. Portal pinned to v1.6.0 in `apps/api/package.json` + `apps/web/package.json` as part of PR 07-5 (commit `27aed23`); local payload type declarations replaced with shared imports.
9. ✅ Heroes consumes the new contract end-to-end. Cutover EXECUTED 2026-05-05 against prod directly; Heroes Deploy A LIVE IN PROD on sha `f62f2be` (Deploy run `25314176044`). Cutover-verify 4/4 automated checks PASS; rebroadcast `dispatched:72, skipped:0, failed:0`. Heroes' webhook handlers (`packages/server/src/services/portal-events/`) materialise `heroes_profiles` + `email_cache` + `taxonomy_cache` + `user_config_cache` from the Spec 07 envelope on every event; payload-projection helpers exhaustively unit-tested.
...
- **PR 07-3:** ✅ SHIPPED 2026-05-04 (commit `8ca124c`). PATCH `/v1/employees/:id` fires `emitEmploymentUpdated` on real HR-field deltas (suppressed for no-op writes). New `apps/api/src/services/employment-resolution.ts` resolves the block via taxonomy join. `emitUserProvisioned` carries the full Spec 07 envelope (`user`, `employment`, `contactEmail`, `appConfig`) alongside legacy `email`/`appRole`/`branch` (dual-emit at the time; `email`/`branch` dropped in PR 07-5). Race-window regression tests assert one `taxonomy.upserted` envelope per `(taxonomyId, batchId)`. 22 new tests added across 3 files; isolated API suite 519 pass / 0 fail. **`ENABLE_TAXONOMY_EVENTS` flipped to `true` in production 2026-05-05** during cutover; end-to-end delivery verified the same day.
- **PR 07-4:** Publish `@coms-portal/shared` v1.6.0 with new types.
- (Spec 08 Heroes-side work happens in parallel after PR 07-4 lands.)
- **PR 07-5:** ✅ SHIPPED 2026-05-06 (commit `27aed23`, CI run `25415453742`). Burn-in gate waived after day-1 fixes stabilised. Dropped legacy duplicates `email` and `branch` (kept `appRole` — re-classified as the canonical per-app role broadcast since the v1.6.0 envelope has no role field). Bumped `@coms-portal/shared` git+url pin v1.5.0 → v1.6.0; replaced local payload type declarations in `taxonomy-events.ts` / `employment-resolution.ts` / `provisioning-events.ts` with shared imports. Forced manifest `schemaVersion: 2` floor (column default, `registerApp` validator, route Elysia schema; migration `0033_nifty_raza.sql` forward-fills v1 rows). 555/555 API tests pass. Same-day follow-up `29966d7` (migration `0034_drop_heroes_deadconfig.sql`) emptied Heroes' `app_manifests.config_schema` after `leaderboard_eligible` + `starting_points` were verified to have zero consumers in the Heroes repo.
