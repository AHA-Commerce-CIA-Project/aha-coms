...
| 06 | Dual-Email Auth (workspace OIDC + personal OTP) | Portal | PR A shipped 2026-04-30 (commit `049008d`); PRs B–E pending (~1.5–2 weeks portal-side); PR F (spec sweep) pending | Minimal — Heroes compiles unchanged against `@coms-portal/shared` v1.5.0; no required Heroes work until PR F lands | **Yes — blocks Heroes-side rev3 adoption** |
...
- **Pending — Spec 06 PRs C–E:** C login UI (personal-email entry + OTP challenge surfaces), D profile UI (self-service personal-email binding, manage-emails page) + admin user-create/detail/CSV-collision UIs + admin find-by-email, E extras (sign-out-everywhere, active-sessions panel, super_admin one-time login link — requires extending `portalRole` enum first). These PRs unblock Heroes-side rev3 adoption together with PR F.
