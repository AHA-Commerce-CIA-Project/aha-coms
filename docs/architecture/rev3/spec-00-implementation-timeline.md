...
- **Pending — Spec 06 PRs B–E:** OTP service (Brevo integration, token store, rate limiting), login UI (personal-email entry + OTP challenge surfaces), profile UI (self-service personal-email binding, manage-emails page), and extras (admin tooling, find-by-email, sign-out-everywhere, super_admin one-time login link). These PRs unblock Heroes-side rev3 adoption together with PR F.
