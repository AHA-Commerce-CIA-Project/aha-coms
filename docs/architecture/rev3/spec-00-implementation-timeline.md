...
## Status — 2026-05-03 (Specs 01 + 02 + 03 + 03b + 03c + 06 portal-side shipped end-to-end; Spec 02 Phase 4 Heroes adoption shipped; **Spec 06 PR F spec-sweep in flight, then Heroes-side rev3 adoption unblocks**)
...
| 06 | Dual-Email Auth (workspace OIDC + personal OTP) | Portal | PRs A → E shipped 2026-04-30 → 2026-05-03 (commits `049008d`, `6938553`, `ac2f905`, `df1c90c`+`6f96634`, `a684fa9`, `212e103`). PR F (spec sweep) in flight. | Minimal — Heroes compiles unchanged against `@coms-portal/shared` v1.5.0; no required Heroes work until PR F lands | **Yes — blocks Heroes-side rev3 adoption (until PR F merges)** |
...
- **Pending — Spec 06 PR E:** Extras: sign-out-everywhere (#9), active-sessions panel (#10), super_admin one-time login link (#11 — requires extending `portalRole` enum first). Incidental fix to fold in: global error handler returns 500 for `throw status(401, ...)` from `authPlugin` — observable on every `/api/v1/*` and `/api/me/*` route when unauthenticated (Cloud Run logs show correct internal 401, response goes out as 500). `/api/userinfo` is unaffected (uses `set.status = 401` directly). Pre-existing across the entire authPlugin namespace; folding into PR E's self-merge gate gives security review on a small global-error-handler tweak alongside the one-time-login-link work. PR E unblocks Heroes-side rev3 adoption together with PR F.
- **Pending — Spec 06 PR F (spec sweep):** Updates this doc + spec-01 + spec-03 + heroes-integration-handoff to reflect the full shipped state of PRs A–E. PR F landing is the gate that unblocks Heroes-side rev3 adoption.
...
4. **Rev 3 Spec 06** — Dual-email auth. **In flight (PRs A, B1, B2, C shipped 2026-04-30 → 2026-05-01; D-F pending).** Multi-row `identity_user_emails` table replaces single-column model; OTP path for any email provider via Brevo (smoke-tested end-to-end on prod 2026-05-01); self-service personal-email binding; admin tooling (find-by-email, sign-out-everywhere, super_admin one-time login link). 6 PRs (A-F); F is the spec-update sweep that unblocks Heroes-side rev3.
