...
## Status — 2026-05-04 (Specs 01 + 02 + 03 + 03b + 03c + 06 portal-side shipped end-to-end; Spec 07 PRs 07-1 / 07-2 / 07-3 / 07-4 SHIPPED 2026-05-04 — `@coms-portal/shared` v1.6.0 published. Spec 08 Heroes-side **PR A1 SHIPPED 2026-05-04** (schema migration only). Next focus: **Spec 08 PR A2** — Heroes behaviour rewrite (10-handler webhook split + broker-exchange upsert + sheet-sync rewrite + cutover scripts), then coordinated cutover window + portal PR 07-5.)
...
| 07 | Org Taxonomies & Employment Block (`@coms-portal/shared` v1.6.0 contract bump) | Portal | Drafted 2026-05-04. PR 07-1 SHIPPED 2026-05-04 (commit `26057ec`) — schema + seed + manifest v2 + cutover rollback companion. PR 07-2 SHIPPED 2026-05-04 (commit `66b0a52`) — sync endpoint + admin UI + gated emitters (`ENABLE_TAXONOMY_EVENTS` off by default) + `taxonomy_edit_locks` migration. PR 07-3 SHIPPED 2026-05-04 (commit `8ca124c`) — `employment-resolution` service, PATCH employees emits `employment.updated` on HR delta, `user.provisioned` carries the full Spec 07 envelope alongside legacy fields (dual-emit). PR 07-4 SHIPPED 2026-05-04 (commit `19cf057` on `mrdoorba/coms-shared`, tag `v1.6.0`) — `EmploymentBlock`, `TaxonomyRef`, `TaxonomyEvent`, `WebhookUserEnvelope`, `ContactEmail`, `taxonomies?: string[]` manifest extension; v1.5.0 backcompat locked by 12 verbatim tests. PR 07-5 (drop legacy emit, gated on Heroes Deploy A) remaining. | No (sets up the contract Heroes consumes in Spec 08) | **Yes — precondition for Spec 08 + every future H-app onboarding** |
| 08 | Heroes Spec 03 Cutover Protocol (one-time migration plan) | Heroes | Drafted 2026-05-04 — 2 PRs (~2 weeks Heroes engineering). **PR A1 SHIPPED 2026-05-04 (commit `57fd523` on `coms_aha_heroes/main`)** — schema migration: rename `users → heroes_profiles`, drop `branches`/`teams`/`authUser`/`user_emails`, add 6 portal-projection caches (`pending_alias_resolution`, `alias_cache`, `taxonomy_cache`, `user_config_cache`, `email_cache`, `deactivated_user_ingest_audit`), retarget 10 dependent FKs to `heroes_profiles`, retarget better-auth `session.userId`/`account.userId` `text → uuid`, pin `@coms-portal/shared` v1.6.0. Manual SQL warding (IF EXISTS on DROP CONSTRAINT, in-migration TRUNCATE before text→uuid + new FKs). `@coms/shared` typecheck clean via three stub files (`auth/session.ts`, `db/seed/auth.ts`, `db/seed/dev.ts`); A1 stops at the schema layer. **PR A2 (~1 week, behaviour rewrite) remaining** — webhook handler split into 10 modules under `services/portal-events/`, broker-exchange upsert against `heroes_profiles`, sheet-sync rewrite via `POST /api/aliases/resolve-batch`, pull-on-boot `taxonomies/sync`, `bun run cutover:verify` script, `POST /admin/pending-aliases/sweep`, CI guard against `INSERT INTO users`/`heroes_profiles` patterns, plus rewire of ~25 server/web files that still import dropped symbols (29 server typecheck errors expected until A2 lands). Cutover (Phase 4) gated on PR A2 + portal PR 07-5. | **Yes — the entire spec is Heroes work** | **Yes — must land before Heroes takes real users** |
