...
> - **PR E — pending.** Extras: sign-out-everywhere, active-sessions panel, OTP-bypass / one-time login link. **Incidental fix to fold in:** global error handler returns 500 for routes that `throw status(401, ...)` from `authPlugin` — observable on every `/api/v1/*` and `/api/me/*` route when unauthenticated (Cloud Run logs show the correct internal 401, response goes out as 500). `/api/userinfo` is unaffected because it manually sets `set.status = 401`. Pre-existing across the entire authPlugin namespace; folding into PR E's self-merge gate gives security review on a small global-error-handler tweak alongside the one-time-login-link work.
