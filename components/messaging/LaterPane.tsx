...
// LaterPane — Saved messages / saved tasks / posted cards experience
// extracted from the legacy /later page. Used by the unified /messages
// workspace (when ?later=<tab> is set) AND by the standalone /later page.
//
// Caller may pass `tabOverride` to control the active tab; otherwise the
// component falls back to the URL's `?tab=...` for backwards compatibility.
...
interface LaterPaneProps {
  /** When set, overrides the URL's ?tab= for the active tab — used by the
   *  unified /messages workspace which encodes the active later tab as
   *  ?later=<tab> on its own URL instead of ?tab=<tab>. */
  tabOverride?: 'messages' | 'tasks' | 'posted-cards';
  ...
  onTabChange?: (tab: 'messages' | 'tasks' | 'posted-cards') => void;
}
...
export function LaterPane({ tabOverride, onTabChange }: LaterPaneProps = {}) {
  ...
  // Resolve initial tab: prop wins, else ?tab=, else default to 'messages'.
  const initialTab = (() => {
    if (tabOverride) return tabOverride;
    const t = params.get('tab');
    return t === 'tasks' || t === 'posted-cards' ? t : 'messages';
  })();
  const [tab, setTabState] = useState<'messages' | 'tasks' | 'posted-cards'>(initialTab);
  // Local setter — also fires the parent's onTabChange so the URL updates.
  const setTab = (t: 'messages' | 'tasks' | 'posted-cards') => {
    setTabState(t);
    onTabChange?.(t);
  };
  ...
  // Keep the active tab in sync with the override (unified workspace) or
  // the URL ?tab= (standalone /later page) so clicking another Later sub-item
  // updates the view without a full remount.
  useEffect(() => {
    if (tabOverride) {
      setTabState(tabOverride);
      return;
    }
    const t = params.get('tab');
    if (t === 'tasks' || t === 'posted-cards' || t === 'messages') {
      setTabState(t);
    }
  }, [params, tabOverride]);
