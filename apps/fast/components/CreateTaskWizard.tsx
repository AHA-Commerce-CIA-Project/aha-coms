...
interface CreateTaskWizardProps {
    open: boolean;
    onClose: () => void;
    onSubmitted?: (result: { id: string; taskToken: string; assigneeId: string }) => void;
}
...
const STEP_LABELS = ['Assignment Details', 'Priority & Description', 'Review & Submit'];
const STEP_SUBTITLES = [
    'Pick the assignee, optionally add request type, brand code, and title',
    'How urgent is this and describe the details',
    'Review your task before assigning',
];
...
export function CreateTaskWizard({ open, onClose, onSubmitted }: CreateTaskWizardProps) {
    ...
    const validateStep = (step: number): boolean => {
        const errors: string[] = [];
        if (step === 1) {
            if (!formData.assigneeId) errors.push('Please select an assignee.');
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.assigneeId) { setStepErrors(['Please select an assignee.']); setCurrentStep(1); return; }
        ...
        try {
            ...
            const fileUrls = [
                ...images.slice(1).map((img) => img.url),
                ...formData.fileUrls,
            ];
            ...
            const res = await fetch('/fast/api/tasks', {
                ...
                body: JSON.stringify({
                    ...
                    dueDateTime: '',
                    requestType: formData.requestType || 'internal',
                    imageUrl,
                    fileUrls,
            ...
            onSubmitted?.({ id: data.id, taskToken: data.taskToken, assigneeId: formData.assigneeId });
            onClose();
        } catch (err: any) {
            ...
            >
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">Assign a task directly to a team member — auto-accepted, lands in their Direct Requests.</p>
                    </div>
                    <button
                        type="button"
                        onClick={() => !submitting && onClose()}
                        ...
                        <>
                            <div className="space-y-1.5">
                                ...
                                <p className="text-xs text-slate-400">
                                    The task will be auto-accepted and appear in this member&apos;s Direct Requests tab.
                                </p>
                            </div>
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            {REQUEST_TYPES.map((rt) => (
                                                <label key={rt.value} className="flex items-center gap-2 cursor-pointer">
                                                    <input
                                                        type="radio" name="ct-requestType"
                                                        value={rt.value}
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
                                ...
                                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                    ...
                                    <div className="text-slate-500">Division (Requester)</div>
                    ...
                    {editingFromReview && currentStep < 3 ? (
                        ...
                        <>
                            ...
                            {currentStep < 3 ? (
                                ...
                                >
                                    {submitting ? (<><Loader2 className="w-4 h-4 animate-spin" /> Creating…</>) : (<>Create &amp; Assign <Send className="w-4 h-4" /></>)}
                                </button>
                            )}
                        </>
                    )}
