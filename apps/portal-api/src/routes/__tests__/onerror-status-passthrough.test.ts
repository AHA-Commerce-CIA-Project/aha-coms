 ...
 * unchanged.  Before this fix (spec-06 PR E), the onError handler in apps/portal-api/src/index.ts
 ...
 *
 * The handler is extracted into apps/portal-api/src/middleware/api-error-handler.ts so this
...
test('mirrors authPlugin: derive throws 401, route never executes', async () => {
  ...
  // apps/portal-api/src/middleware/auth.ts:89 throws status(401, ...) when no session cookie.
