    ...
    try {
      ...
      if (isSuspendedOrArchived) {
        if (portalUser && portalUser.status === 'active') {
          ...
          if (!dryRun && portalUser.gipUid) {
            await getAuth().updateUser(portalUser.gipUid, { disabled: true })
          }
      ...
      if (portalUser) {
        if (portalUser.status === 'inactive') {
          ...
          if (!dryRun && portalUser.gipUid) {
            await getAuth().updateUser(portalUser.gipUid, { disabled: false })
          }
        ...
        if (!dryRun) {
          // Check if GIP user already exists before creating
          let gipUid: string

          try {
            const existingGipUser = await getAuth().getUserByEmail(email)
            gipUid = existingGipUser.uid
          } catch (gipLookupError: unknown) {
            const isNotFound =
              gipLookupError instanceof Error &&
              'errorInfo' in gipLookupError &&
              (gipLookupError as { errorInfo: { code: string } }).errorInfo.code ===
                'auth/user-not-found'

            if (!isNotFound) {
              throw gipLookupError
            }

            const createdGipUser = await getAuth().createUser({
              email,
              displayName: name || email,
              emailVerified: false,
            })
            gipUid = createdGipUser.uid
          }
