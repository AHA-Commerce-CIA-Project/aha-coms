...
- [~] **T47: End-to-end smoke test**
  ...
  - **Checklist:**
    ...
    - Mobile chrome: install PWA, log in, verify chrome looks identical to portal's. — **PARTIAL** (chrome parity verified after Findings 2 + 3 + 5 mended; PWA install prompt remains — deferred to FU-6)
    - Admin operations: HR sync, audit log, settings. — **NOT EXERCISED** (the admin loads are reachable post-Finding-1 fix; an operator-driven write — trigger sheet sync / update a setting / file audit-log entry — would close this row in the next session)
  - **Acceptance:** Every checklist item passes.
  - **What remains in the next session:** (1) Run one admin write operation on heroes (Admin → Sheet Sync → "Trigger now" is the canonical exercise — confirms `services/sheet-sync` end-to-end including the post-Phase-5 `heroes_profiles.can_submit_points` read path). (2) Decide whether to formally seal T47 with PWA install deferred to FU-6, or hold T47 open until FU-6 closes. The CP11 acceptance criteria reference §§ 1–9 + 11–14 of the integration contract, which the chrome-parity + auth + admin paths already cover; the PWA install is a packaging concern outside that scope.
