...
- [~] **T47: End-to-end smoke test**
  ...
  **Finding 3 (deferred — design call, not a bug):** Portal-web's MobileBottomNav surfaces only 3 of 7 admin sections (Dashboard, Employees, Apps); Teams, Aliases, Taxonomies, Audit Log are unreachable on mobile. This is deliberate per `apps/portal-web/src/routes/(authed)/+layout.svelte:47-53` comment: *"keep the user-visible items short; admin tools live behind the desktop sidebar, not the mobile bottom strip."* The smoke surfaces the design tension but the design call itself is out of T47 scope. Revisit options: (a) overflow menu on the bottom nav, (b) horizontal scroll, (c) move full admin nav into AccountWidget dropdown. Track as a future UX iteration if/when admins regularly hit mobile.
