...
> Last updated: 2026-05-12 (Checkpoint 4 crossed; CP4 Findings settled; two cross-cutting Phase 5 follow-ups recorded for a fresh session — FU-1 health probe, FU-2 orphan cloudbuild secrets)
...
- [ ] **FU-1: Diagnose the "Degraded" status on the HEROES dashboard card**
  ...
  - **Diagnostic steps for the fresh session:**
    1. Check current status: portal-api logs around the probe interval (`gcloud logging read 'resource.type="cloud_run_revision" AND resource.labels.service_name="coms-portal-api" AND textPayload=~"health"' --project fbi-dev-484410 --freshness=30m`).
    2. Look at the probe implementation: `apps/portal-api/src/services/health-probe.ts` (URL construction logic).
    3. Decide between (a) adding `healthCheckUrl` field to `app_registry` schema + migration, defaulted to `${app.url}/api/health` for legacy apps, set explicitly to `https://coms-heroes-api-45tyczfska-et.a.run.app/api/health` for heroes; OR (b) keeping the convention-based derivation but using the new `HEROES_WEBHOOK_URL` semantics (the api host) instead of `HEROES_APP_URL` (the web host).
  - **Acceptance:** HEROES card on dashboard shows green / healthy status. portal-api logs show probe returning 200 from heroes-api.
...
- [ ] **FU-2: `apps/portal-web/cloudbuild.yaml` references three non-existent secrets**
  ...
  - **Two fix shapes — pick one:**
    1. **Provision the three orphan secrets.** Create `coms-portal-vite-gip-api-key` as an alias-or-copy of `coms-portal-gip-api-key`, and `coms-portal-vite-gip-auth-domain` + `coms-portal-vite-gip-project-id` as plain-string secrets carrying the project's Firebase web config. Grant `roles/secretmanager.secretAccessor` on each to the Cloud Build service account (`<project-number>@cloudbuild.gserviceaccount.com`). Operationally heavy (three new secrets to rotate when the API key rotates).
    2. **Rewrite `apps/portal-web/cloudbuild.yaml` to mirror the GHA workflow.** Replace the three `availableSecrets` entries with a single fetch of `coms-portal-gip-api-key` and inline the two plain values. Less infrastructure, single source of truth for the API key, matches the GHA pattern exactly. Recommended.
  - **Acceptance:** Either `gcloud secrets list --project fbi-dev-484410 | grep coms-portal-vite-gip` returns three rows AND a test `gcloud builds submit --config apps/portal-web/cloudbuild.yaml .` succeeds; OR `apps/portal-web/cloudbuild.yaml` no longer references the three orphan secrets AND a test build succeeds. Same shape verification applies to `apps/heroes-{api,web}/cloudbuild.yaml` and `apps/portal-api/cloudbuild.yaml` if they have analogous orphan references — sweep before committing.
