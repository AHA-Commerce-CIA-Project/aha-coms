...
- [ ] **FU-7: Portal-api test suite has ~196 pre-existing failures**
  - **Where it surfaced:** T47 work hit the test suite when validating the `/api/userinfo` change for Finding 5. Running `bun test` in `apps/portal-api` reports `379 pass, 196 fail` across 575 tests / 63 files. Git stash confirmed the 196 fails predate the T47 work (same numbers without my changes applied).
  - **Why it's tracked separately:** the workspace-level `bun run test` from the monorepo root delegates to `bun --filter @coms-portal/portal-api test`, but somewhere between this session's earlier `bun run test` (which reported all-pass per the smaller per-file slices it showed) and the full-suite run, the 196 latent fails surfaced. Either the per-file invocation skipped failing files, or new tests were added during Phase 1–5 that don't run in CI.
  - **Spread observed in the fails:** `POST /api/v1/apps/:id/webhooks/:endpointId/reactivate`, `GET /users/:portalSub/config/:appId`, `POST /apps/:id/manifest`, `POST /apps/:id/smoketest`, `POST /aliases/resolve-batch`, `GET /taxonomies/sync`, and others. Several look like webhook + manifest + alias paths that didn't move during Phase 5 — these tests likely fail against today's `app_registry` / `app_manifests` shape, possibly because of fixture drift.
  - **Shape to implement:**
    1. Run `bun test` in `apps/portal-api` and capture the full failure list (the 196 fails span ~12 test files based on the spread above).
    2. Triage: bucket the fails into (a) genuine regressions caught by tests, (b) fixture drift the tests didn't auto-track when schemas evolved, (c) tests against routes that no longer exist or were renamed.
    3. Fix or quarantine accordingly. The CI workflow (`.github/workflows/ci.yml`) presumably gates merges on a green test run — the fact that 196 fails sit on `main` means CI is either skipping them or has been merging through.
    4. Audit `.github/workflows/ci.yml` to confirm what filter / scope runs on PRs. If it runs the full suite, the test failures should have been blocking merges; if it runs a narrower scope, the scope is too narrow.
  - **Acceptance:** Either all 196 fails resolve, or each one carries a `it.skip` annotation with a comment explaining why. CI runs the full suite. Future fails count is tracked at zero.
  - **Why this is FU-rank and not T47 scope:** Phase 5's work didn't introduce these. T47 surfacing them is incidental — the smoke ran without touching the failing endpoints. They've been silently broken since before the cache cleanup. Scope-wise, this is closer to a CI / test-suite hygiene task than a Phase 6 verification gate.
  - **Related:** the test-suite-quality entries in agent-skills (`agent-skills:test`, `agent-skills:test-driven-development`); FU-3 (the migrate-step that runs on every push) which presumably is unrelated but shares the deploy-workflow context.
