...
- [~] **T47: End-to-end smoke test**
  ...
  **Finding 4 (deferred — browser-specific, low priority):** No PWA install prompt on the operator's mobile browser (Brave). Brave + Chrome require a service worker + a web manifest meeting installability rules (icons in 192/512 sizes, `start_url`, `display`, etc.). Portal-web has the service worker (T20 finding 2's downgraded analysis confirmed it). The manifest may not be meeting the install criteria; needs a Lighthouse PWA audit pass. Not blocking T47's chrome-parity question.
