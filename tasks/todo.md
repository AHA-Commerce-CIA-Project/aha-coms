...
- [ ] **FU-6: Portal-web PWA install prompt not firing**
  - **Where it surfaced:** T47 step 4 (mobile chrome smoke). Operator on Brave mobile, navigating to `https://aha-coms.web.app/`, no install banner / Add-to-Home-Screen prompt. Chrome desktop on the same URL also doesn't prompt; the install icon in the address bar may or may not be visible depending on prior dismissal state.
  - **Why it's not gating T47:** the chrome-parity smoke (single bar on mobile, AccountWidget launcher carries cross-app navigation, slide-over surfaces full admin nav) all passed without the PWA install needing to work. Installability is a packaging concern, not a chrome correctness concern.
  - **Hypothesis:** portal-web has the service worker registered (T20 Finding 2's downgraded analysis confirmed `apps/portal-web/src/service-worker.ts` populates a cache on install and serves cached responses on fetch). The other half of installability is the web manifest at `apps/portal-web/static/manifest.json` (or wherever it lives) — Brave/Chrome require: (a) `start_url`, (b) `display: standalone` or `fullscreen`, (c) `name` + `short_name`, (d) at least one icon entry sized 192×192 + 512×512, (e) `theme_color` + `background_color`. A Lighthouse PWA audit (`chrome://discards` → DevTools → Lighthouse → Progressive Web App category) lists exactly which criteria fail.
  - **Shape to implement:**
    1. Run Lighthouse PWA audit on `https://aha-coms.web.app/` in Chrome (desktop is fine; the criteria are origin-level, not device-specific).
    2. Fix whichever installability criteria the audit flags. Most common gaps in projects with an existing SW: missing manifest entries, missing icon sizes, missing `start_url`, or the manifest file itself not linked from `<head>`.
    3. Re-audit; install prompt should fire on next visit (Brave's heuristics may require 2+ visits across separate days even with full installability — note this when the prompt doesn't fire immediately after the fix).
    4. Verify on real device: install the PWA on phone, open from home screen, confirm splash colour + icon + standalone (no browser chrome) all render.
    5. Apply the same audit to heroes-web — `apps/heroes-web/static/manifest.json` likely has similar gaps. Both apps should be installable independently since they live at different paths.
  - **Acceptance:** Lighthouse PWA audit passes (or notes only Brave-specific heuristic timing). Operator can install both portal and heroes as PWAs from a phone.
  - **Related:** T20 Finding 2 (the service worker analysis); T47 Finding 4 (where the gap surfaced).
