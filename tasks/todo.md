...
- [ ] **FU-8: Operator-driven post-cleanup runtime verification — sheet-sync admin write + heroes p50/p95 + on-device PWA install**
  ...
  - **Three checks to run when an operator is on a phone or laptop with admin access:**
    1. **Sheet-sync admin write (T47 carry-over).** From a signed-in browser, navigate to `https://aha-coms.web.app/heroes/admin/sheet-sync` → click "Trigger now". Expected: the trigger returns success without 5xx; the next page load reflects the most recent sync in the audit log. Why this matters: confirms `services/sheet-sync` end-to-end including the post-Phase-5 `heroes_profiles.can_submit_points` read path that replaced the `user_config_cache.config->>'canSubmitPoints'` shape — the only auth-adjacent code path Phase 5 touched that hasn't been exercised in prod since the migration. If the trigger fails, the sheet-sync code's expectation of `canSubmitPoints` shape is the first suspect (the `boolean | null` → `boolean` tightening from T46 is the most likely break point).
