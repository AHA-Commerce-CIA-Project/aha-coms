...
> Last updated: 2026-05-12 (Checkpoint 4 crossed — Phase 5 sealed, three Findings flagged for Spec 02 Phase 1)
...
1. **`app_registry.url` for heroes points at the destroyed `coms-aha-heroes-app-…run.app`.** Clicking the HEROES card on the portal dashboard POSTs to `aha-coms.web.app/heroes` (a portal-web launch handler that mints a `portal_code`), which 302s the browser to the registered launch URL — that URL was set by the T15-era `register:heroes` script before T16.5 destroyed the combined heroes Cloud Run service and replaced it with `coms-heroes-api` + `coms-heroes-web`. The launch URL is now a dead host. Fix: re-register heroes with `HEROES_APP_URL=https://aha-coms.web.app/heroes` (routing-layer path, not run.app URL) so launches stay same-origin and share `__session` automatically. Two related fields also need updating from T16.5: `HEROES_APP_SA=coms-heroes-api-sa@fbi-dev-484410.iam.gserviceaccount.com` (was `coms-aha-heroes-run-sa@…`); `HEROES_BROKER_ORIGIN=https://aha-coms.web.app` (was the portal run.app URL). The current `register:heroes` script at `apps/portal-api/scripts/register-heroes.ts:128-133` early-returns when the row already exists — generalising to upsert is part of the fix, not just running the script with new env. Lands as a standalone commit, not coupled to T21.

2. **portal-web's service worker intercepts every `/heroes/*` navigation on `aha-coms.web.app`** and currently serves cached 404 responses without reaching heroes-web Cloud Run. The SW was registered at the routing-layer origin during portal-web sign-in flows; its scope is `/` which covers all paths. The `link:` header on the cached response references `../_app/immutable/assets/Toaster.imULxUqb.css` — Toaster is a portal-web component, confirming this is portal-web's SW. After T24+T27 land base-path migration and the `/heroes/*` URL is supposed to render heroes-web pages, this SW will silently block every navigation — every request returns portal-web's cached 404. Fix options (decide before T24 begins): narrow the SW's scope to `/login`, `/dashboard`, and other portal-only paths via `RegisterRoute` / `denylist`; or migrate portal-web off SvelteKitPWA on the routing-layer origin entirely; or unregister the SW for users currently carrying it (one-shot cleanup script). The decision belongs in the Spec 02 Phase 1 ramp because T24 is when it becomes load-bearing.
