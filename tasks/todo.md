...
3. **Heroes registration in portal's `app_registry` is not reproducible from the repo.** The smoke required hand-written SQL inserts into `app_registry`, `app_manifests`, `teams`, `team_members`, `team_app_access`, and `member_app_role` — six tables across three concerns (app identity, team-based access, per-user role). A future fresh-laptop setup would have to redo all of this. The hand-SQL also **skipped the webhook channel entirely**: no `app_webhook_endpoints` row exists for heroes, no HMAC signing secret was provisioned, and no `user.provisioned` event ever fired — so heroes' `heroes_profiles.role` stayed NULL for `handers.the` (the user is "admin" in portal but heroes' admin UI does not light up, because heroes reads the role from its own `heroes_profiles.role` column populated by the webhook handler at `apps/heroes-api/src/services/portal-events/handle-user-provisioned.ts`). **Fix:** add `apps/api/scripts/register-heroes.ts` modeled on `spec07-register-fast.ts` that lands the full chain in one transaction — app_registry + app_manifests + app_webhook_endpoints (with a dev HMAC secret) — and then a companion `bootstrap-heroes-membership.ts` (or extend `seed-admin.ts`) that creates the seeded team, adds the bootstrap admin to it, grants the team `team_app_access` to heroes, and either emits a `user.provisioned` webhook to heroes' endpoint or invokes portal's grant API so the event fires naturally. The smoke is only fully scripted when heroes' `heroes_profiles.role` lands without any hand-UPDATE.
