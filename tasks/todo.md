...
- [ ] **FU-8: Operator-driven post-cleanup runtime verification — sheet-sync admin write + heroes p50/p95 + on-device PWA install**
  ...
  - **Three checks to run when an operator is on a phone or laptop with admin access:**
    ...
    3. **On-device PWA install verification (FU-6 carry-over).** On a mobile device (Brave + Chrome both, separately): visit `https://aha-coms.web.app/` → confirm install prompt or address-bar install icon appears; install; open from home screen; confirm splash colour (`#0a0a0a` portal) + icon (gradient blue + "C") + standalone chrome render. Repeat for `https://aha-coms.web.app/heroes/` → expect a SEPARATE install prompt (heroes lives in its own scope/start_url), splash colour `#09090b`, gold-gradient + "H" icon, standalone chrome. Brave's heuristics may defer the first prompt for 2+ visits across separate days even on a fully compliant manifest — Lighthouse PWA audit run on either URL surfaces the criteria-level pass/fail and is the authoritative signal.
  - **Acceptance for CP11:** all three rows report green (or with the documented Brave-heuristic-deferral caveat acknowledged). At that point CP11 crosses and Spec 02 seals.
...
- [ ] **FU-9: Sheet-sync trigger silent-fail — IaC drift on env vars + fire-and-forget handler swallows the throw (T47 Finding 6)**
  ...
  - **Proper structural fix (multi-step, requires an operator window):**
    1. **Create Secret Manager entries** for the two sheet IDs and four sheet-tab names (or consolidate into a single JSON-shaped secret with all six values — fewer secrets to manage, one accessor grant). The sheet IDs are semi-sensitive (they identify the canonical spreadsheets) and shouldn't live in a committed tfvars file; Secret Manager is where they belong.
    2. **Grant the heroes-api runtime SA** (`coms-aha-heroes-run-sa` or whatever the current SA name is) the `roles/secretmanager.secretAccessor` role on each new secret. Mirrors the existing `GOOGLE_SHEETS_SA_KEY` + `DATABASE_URL` shape.
    3. **Rewrite `infra/heroes/cloud-run.tf:244-258`** to use `value_source.secret_key_ref` per env var instead of the `dynamic "env"` gated on the variable. Remove the `sheet_id_points` + `sheet_id_employees` variables from `variables.tf` (or keep them as overrides if the dev path needs literal values; secret-backed for prod).
    4. **`tofu apply` from `infra/heroes/`** per the laptop-CLI runbook in `infra/README.md`. New revision deploys with env vars sourced from Secret Manager.
    5. **Operator re-runs Sync Now** on `https://aha-coms.web.app/heroes/admin/sheet-sync`. Expected: status flips to `running`, then back to `idle`; new row appears at top of Sync History with today's date; Last sync timestamp updates to today.
    6. **Optional cleanup of the silent-fail.** Either (a) add a synchronous validation in the route handler BEFORE calling `triggerSyncInBackground` (env var check → return 4xx if missing), so the operator gets immediate feedback instead of a deceptive 200; or (b) keep fire-and-forget but write a `failed` row to `sheet_sync_jobs` from inside the `.catch` so at least the failure surfaces in the UI's history table. Both are improvements; (b) is closer to "least surprise" since the existing Cloud Run logs already capture the throw.
  ...
  - **Why the IaC drift went unnoticed until today:** standing-principle 1's "manual-console-state vs IaC-state drift" risk realized in production. The May 8 successful sync was OOB-edited and never reconciled into Tofu source. Every subsequent apply silently dropped the env vars; the next sync attempt was today; the silent-fail handler hid the symptom until an operator looked at the Sync History closely. This is a worth-recording case study for the standing principle.
  - **Acceptance:** all three layers closed. Sheet IDs live in Secret Manager + are referenced via `secret_key_ref` in `cloud-run.tf`. The silent-fail surfaces failures into Sync History (either via 4xx on validation or via `failed` row from the catch). The cosmetic i18n fix is already in this session's commit; the remaining two are operator-driven IaC + code work in the next session.
...
- [ ] **CHECKPOINT 11**: Heroes is the reference implementation for the COMS integration contract — for §§ 1–9 and §§ 11–14. Notifications (§10) remain the documented deviation, awaiting the platform-notifications spec.
  - **Gating row at session close 2026-05-13:** FU-8 (operator-driven post-cleanup runtime verification — sheet-sync admin write + heroes p50/p95 + on-device PWA install). T47, T48, T49 have all sealed `[x]`; the contract-gate verifications they cover are green. CP11 crosses when an operator runs FU-8's three checks against prod and reports each row green.
