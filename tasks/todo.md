...
> Last updated: 2026-05-12 (T19 sealed — `https://aha-coms.web.app` live, four corridors routed, T20 cookie verification gated on operator's browser)
...
- [~] **T20: Verify cross-app cookie sharing** *(operator-gated, manual DevTools)*
  - **Prerequisites:** T19 ✓
  ...
  - **Steps:**
    1. Open `https://aha-coms.web.app/login` in a clean browser profile (or Incognito) to avoid stale cookies from the `*.run.app` sign-in.
    2. Complete personal_otp sign-in. After success, portal-web redirects to its dashboard.
    3. Open DevTools → Application → Cookies → `https://aha-coms.web.app`. Confirm `__session` is present, Domain blank (host-only), Path `/`, HttpOnly + Secure + SameSite Lax.
    4. Navigate to `https://aha-coms.web.app/heroes/dashboard`. The page will not render correctly until T24+T29 land base-path migration (heroes-web's routes are still mounted at `/`, not `/heroes/*`) — that is expected. The cookie check is the goal, not the rendered page.
    5. DevTools → Network → click the `/heroes/dashboard` request → Headers → Request Headers → `Cookie:` line should include `__session=<jwt>`.
  ...
  - **What to do if the cookie does not appear on heroes requests:** the browser is treating `aha-coms.web.app/heroes/*` as cross-site somehow. Check the Set-Cookie response from sign-in for an unexpected `Domain=` attribute; check that the sign-in request actually completed via `https://aha-coms.web.app/...` and not via the `*.run.app` URL (which would scope the cookie to the run.app host and break crossing).

- [ ] **CHECKPOINT 4**: Single-origin routing works at `aha-coms.web.app`.
