...
> Last updated: 2026-05-12 (Checkpoint 4 crossed; CP4 Findings settled; FU-1 + FU-2 code-complete in tree, gated on one shared prod apply window)
...
- [~] **FU-1: Diagnose the "Degraded" status on the HEROES dashboard card**
  ...
  - **Acceptance:** HEROES card on dashboard shows green / healthy status. portal-api logs show probe returning 200 from heroes-api. Flip `[~]` → `[x]` after the post-apply dashboard reload.
...
- [~] **FU-2: `apps/portal-web/cloudbuild.yaml` references three non-existent secrets**
  ...
  - **Code-complete 2026-05-12:** Picked option 2 (rewrite over provision). `apps/portal-web/cloudbuild.yaml` now fetches a single `coms-portal-gip-api-key` via `availableSecrets` (matching the GHA workflow) and inlines `VITE_GIP_AUTH_DOMAIN` as a Cloud Build substitution (`_VITE_GIP_AUTH_DOMAIN`) + `VITE_GIP_PROJECT_ID` via the built-in `${PROJECT_ID}`. Comment block at the top of the file names this yaml as the manual escape hatch and points at the GHA workflow as the hot path. Cloud Build's project SA needs `roles/secretmanager.secretAccessor` on `coms-portal-gip-api-key` — added in `infra/iam-portal-runtime.tf` as `google_secret_manager_secret_iam_member.cloud_build_gip_api_key` (sibling to `portal_runtime_gip_api_key`, using `data.google_project.current.number` for the `<project-number>@cloudbuild.gserviceaccount.com` identity). Swept `apps/heroes-{api,web}/cloudbuild.yaml` + `apps/portal-api/cloudbuild.yaml`: none declare `availableSecrets` (no analogous orphans). Verified: `tofu fmt -check -diff` clean, `tofu validate` clean, `tofu plan -target=google_secret_manager_secret_iam_member.cloud_build_gip_api_key` shows `Plan: 1 to add, 0 to change, 0 to destroy` — single IAM binding, no surrounding drift.
  - **Prod apply (pending operator window):**
    1. `cd infra && tofu apply -target=google_secret_manager_secret_iam_member.cloud_build_gip_api_key` (or a plain `tofu apply` if other tracked drift is acceptable to land in the same window). One binding adds.
    2. One-off test build (paid action — operator's call on timing): `gcloud builds submit --config apps/portal-web/cloudbuild.yaml --project=fbi-dev-484410 .`. Expected: build resolves the single secret, bakes it into the image, pushes to `coms-portal-registry`, and `gcloud run deploy`s `coms-portal-web` with a new revision. If the operator does not want to spend the build minutes today, the yaml + IAM are still correct in tree — the verification can wait until the next time the hatch is actually needed.
  - **Acceptance:** `apps/portal-web/cloudbuild.yaml` no longer references the three orphan secrets ✓ (in tree); a test build succeeds (pending operator). Flip `[~]` → `[x]` after the test build returns green, or after the next genuine escape-hatch use succeeds (whichever comes first).
