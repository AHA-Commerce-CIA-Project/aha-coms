...
- [~] **T54: Visual parity check — awaiting operator window**
  ...
  - **Operator runbook:**
    1. From the monorepo root: `bun run --filter @coms-portal/ui-react-smoketest dev` (binds `http://localhost:4400`).
    2. In a second terminal: `bun run dev:heroes-web` (binds heroes-web's default port; visit a signed-in `/heroes/dashboard` for a full chrome render) and/or `bun run dev:portal-web` (binds portal-web's default port).
    3. Open all three side-by-side in Chrome / Brave. For each of the four quadrants below, capture matching screenshots of `ServiceBar`, `Sidebar`, `MobileTopBar`, `MobileBottomNav`, `SlideOverNav` (open the drawer), `AccountWidget` (open the popover):
       - **desktop + light** (≥768px, light theme via ServiceBar toggle).
       - **desktop + dark** (≥768px, dark theme via ServiceBar toggle).
       - **mobile + light** (DevTools mobile emulation ≤640px, light theme).
       - **mobile + dark** (DevTools mobile emulation ≤640px, dark theme).
    4. Land the screenshots + a short divergence note (or "parity confirmed, no divergence") in `packages/ui-react/docs/t54-visual-parity.md` and flip T54 to `[x]`.
  - **Note:** the smoketest's app-level CSS classes (`sidebar-link-active`, `bnav-active`, `section-label`, `tap-active`) are deliberately a minimal-shape mirror in `examples/smoketest/src/app/globals.css` — heroes-web's app.css carries richer rules (medal-rank accents, etc.) that are heroes-internal. Expect the smoketest's "active" treatments to read slightly differently in colour but identical in shape; that's by-design and goes in the divergence list.

- [~] **CHECKPOINT 12: React chrome packages at visual parity with their Svelte siblings — awaiting T54.** Structural verification complete: stub consumer renders every component (HTTP 200 + SSR HTML inspected), `packages/ui-react/` + `packages/account-widget-react/` + `@coms-portal/ui-react-smoketest` all typecheck green, Next.js production build green. Visual parity confirmation awaits the operator window described under T54. After CP12 crosses, Phase 2 (subtree merge fast) opens.
