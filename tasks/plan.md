...
> Last updated: 2026-05-12 (after T17 — GHA workflows, not Cloud Build triggers)
> Status: Phase 1 sealed at `34fbedd`; Phase 3 sealed and Checkpoint 2 crossed at `33593a9`; T16 lands the portal split + per-service Cloud Build scaffolding at `5935d00`; T16.5 splits `infra/heroes/` into `coms-heroes-api` + `coms-heroes-web` with their own least-priv runtime SAs at `de68b28`; the per-app-resources + per-app naming principles in this file are the standing rules going forward (heroes images now route to heroes' own `coms-heroes-repo`); T17 returned deploys to GitHub Actions per the new "CI/CD via GHA while public" principle — four `.github/workflows/deploy-*.yml`, heroes' WIF renamed `coms-heroes-{wif-pool,deployer-sa}` and re-pointed at `mrdoorba/coms-portal`. Next: Checkpoint 3 (per-service deploys verified independent — operator runs the tofu apply for T16.5 + T17 + opens four no-op test PRs to confirm path filters), then Phase 5 (Firebase Hosting staging). Findings 1 + 2 from T15 are closed in T16.
