...
> Last updated: 2026-05-12 (Phase 6 sealed — all six external repos archived (`coms-sdk`, `coms-shared`, `coms-ui`, `coms-design-tokens`, `coms-account-widget`, `coms-aha-heroes`) via `gh repo archive`; T23 rewrote five in-tree `repository.url` fields to `mrdoorba/aha-coms` + per-package `directory` qualifiers. **Checkpoint 5 crossed — Spec 01 complete.** Naming truth-up: the heroes remote is `coms-aha-heroes` (hyphenated), not `coms_aha_heroes` (underscored) as docs had it everywhere; on-disk dir keeps the underscore to preserve subtree-merge history. One Phase 6 finding logged for follow-up: doc rot in `DESIGN_SYSTEM.md` + per-package READMEs/CONTRIBUTINGs still prescribes the polyrepo workflow against now-frozen repos — not blocking Spec 02, absorbed into the next rev-3 doc sweep. Next: Spec 02 Phase 1 (T24 — heroes single-origin migration begins with `kit.paths.base: '/heroes'`).
